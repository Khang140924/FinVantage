...
import { deleteNotification, getNotifications, getUnreadNotificationCount, markAllNotificationsRead, markNotificationRead } from "../services/api.js";
...
export default function NotificationMenu({ refreshKey = 0 }) {
  ...
  const loadCount = useCallback(async () => {
    try { const result = await getUnreadNotificationCount(); setUnreadCount(Number(result.unread_count || 0)); }
    catch { /* The dropdown shows the actionable error when opened. */ }
  }, []);
  ...
  return <div className="relative" ref={ref}>
    <button type="button" className="icon-button relative" title={t("topbar.notifications")} onClick={() => setOpen((value) => !value)} aria-expanded={open}>
      ...
      {unreadCount > 0 && <><span className="absolute right-1 top-1 h-2.5 w-2.5 rounded-full bg-rose-500 ring-2 ring-white dark:ring-slate-900" /><span className="sr-only">{t("notifications.unread", { count: unreadCount })}</span></>}
    </button>
    {open && <section className="absolute right-0 top-full z-50 mt-2 w-[min(24rem,calc(100vw-2rem))] overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xl dark:border-slate-700 dark:bg-slate-900">
      ...
      {loading ? <div className="flex h-48 items-center justify-center"><Loader2 className="h-6 w-6 animate-spin text-emerald-600" /></div> : error ? <div className="p-5 text-center"><p className="text-sm text-rose-600">{error}</p><button className="soft-button mt-3" onClick={loadNotifications}>{t("notifications.retry")}</button></div> : visible.length ? <div className="max-h-[28rem] overflow-y-auto">{visible.map((item) => <article key={item.id} className={`border-b border-slate-100 p-4 dark:border-slate-800 ${item.is_read ? "bg-white dark:bg-slate-900" : "bg-emerald-50/70 dark:bg-emerald-950/20"}`}><div className="flex items-start gap-3"><span className={`mt-1 h-2.5 w-2.5 shrink-0 rounded-full ${item.is_read ? "bg-slate-300" : "bg-emerald-500"}`} /><div className="min-w-0 flex-1"><p className="font-semibold">{item.title}</p><p className="mt-1 text-sm leading-5 text-slate-500">{item.message}</p><p className="mt-2 text-xs text-slate-400">{new Date(item.created_at).toLocaleString("vi-VN")}</p></div><div className="flex shrink-0 gap-1">{!item.is_read && <button className="icon-button h-8 w-8" title={t("notifications.markRead")} onClick={() => markRead(item)}><Check className="h-4 w-4" /></button>}<button className="icon-button h-8 w-8 text-rose-600" title={t("actions.delete")} onClick={() => remove(item.id)}><Trash2 className="h-4 w-4" /></button></div></div></article>)}</div> : <div className="p-8 text-center"><Bell className="mx-auto h-8 w-8 text-slate-300" /><p className="mt-3 text-sm font-semibold text-slate-500">{t("notifications.empty")}</p></div>}
