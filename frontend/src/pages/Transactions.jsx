...
export default function Transactions({ searchQuery, invoices = [], apiStatus = {}, onChanged, onNavigate }) {
  ...
  return <div className="space-y-6">
    <section className="app-card flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div className="flex flex-wrap gap-2"><DataSourceBadge loading={apiStatus.loading} source="backend" /><button type="button" className="soft-button" onClick={exportCsv}><Download className="h-4 w-4" />{t("actions.exportCsv")}</button></div>
    </section>
    ...
    <section className="app-card overflow-hidden">
      ...
      <div className="hidden overflow-x-auto md:block">
        <table className="w-full min-w-[760px] text-left text-sm">
          ...
          <tbody>{filtered.length ? filtered.map((item) => <tr key={item.id} className="table-row">
            <td className="px-4 py-4 font-semibold">{item.store}</td><td className="px-4 py-4 text-slate-500">{item.date}</td><td className="px-4 py-4">{item.category}</td><td className="px-4 py-4 font-bold">{formatCurrency(item.amount, item.currency)}</td><td className="px-4 py-4"><StatusBadge status={item.status} /></td>
      ...
      <div className="divide-y divide-slate-100 dark:divide-slate-800 md:hidden">
        {filtered.length ? filtered.map((item) => <article key={item.id} className="space-y-3 p-4">
          <div className="flex items-start justify-between gap-3"><div><h3 className="font-bold text-slate-950 dark:text-white">{item.store}</h3><p className="mt-1 text-xs text-slate-500">{item.date} · {item.category}</p></div><StatusBadge status={item.status} /></div>
