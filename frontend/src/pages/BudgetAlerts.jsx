...
import { EXPENSE_CATEGORIES, normalizeExpenseCategory } from "@shared/expenseCategories.js";
...
export default function BudgetAlerts({ data = { budgets: [], alerts: [] }, apiStatus = {}, onChanged }) {
  ...
  const existingCategories = useMemo(() => new Set(data.budgets.map((item) => normalizeExpenseCategory(item.category)).filter(Boolean)), [data.budgets]);
  ...
  async function submit(event) {
    ...
    const canonicalCategory = normalizeExpenseCategory(category);
    if (!canonicalCategory) return setError(t("budgets.invalidCategory"));
    if (!Number.isSafeInteger(amount) || amount <= 0) return setError(t("budgets.invalidAmount"));
    setBusy(true);
    try {
      const updating = existingCategories.has(canonicalCategory);
      await saveBudget(canonicalCategory, amount);
  ...
  function edit(budget) {
    setCategory(normalizeExpenseCategory(budget.category) || "Khác"); setAmountDigits(String(Math.round(budget.limit ?? budget.amount))); setEditingId(budget.id); setError(""); setMessage("");
  }
